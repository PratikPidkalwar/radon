...
const { isValid, isValidObjectId, isValidRequestBody, isValidStatus } = require("../utils/util");
...
const updateOrder = async function (req, res) {
  try {
    ...
    if (status == "pending") {
      return res.status(400).send({ status: false, message: "status cannot be pending" })
    }
